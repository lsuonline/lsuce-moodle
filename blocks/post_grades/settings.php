...
if ($ADMIN->fulltree) {
    ...
    // MDL-32984
    $exceptions->set_updatedcallback(function() use ($courses, $exceptions) {
        $ids = $exceptions->get_setting();
        foreach ($ids as $id) {
            $course = $courses[$id];
    ...
    $legal_writing->set_updatedcallback(function() use ($courses, $legal_writing) {
        $ids = $legal_writing->get_setting();
        foreach ($ids as $id) {
            $course = $courses[$id];
