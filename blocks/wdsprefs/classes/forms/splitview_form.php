...
require_once($CFG->dirroot . '/blocks/wdsprefs/classes/forms/formslib.php');
trait Extractor {
...
abstract class split_form extends wds_form {

    protected function forge_split_all($course) {
        $grouping = 1;
        foreach ($course->sections as $section) {
            $_POST['shell_name_'.$grouping.'_hidden'] = $section->section_number;
...
class split_form_select extends split_form {
    ...
    public function definition() {
        ...
        foreach ($semesters as $semester) {
            ...
            foreach ($semester->courses as $course) {
                ...
                if (wds_split::exists($course)) {
                    $display .= ' (' . self::_s('split_option_taken') . ')';
                }

                $key = $semester->id . '_' . $course->id;
                // $key = $semester->academic_period_id . '_' . $course->id;
    ...
    public function validation($data, $files) {
        ...
        $this->next = wds_split::exists($course) ? self::UPDATE : $this->next;
...
class split_form_shells extends split_form {
    ...
    public static function build($semesters) {
        ...
        return array('course' => self::stupid_course($semesters, $semid, $couid));
    ...
    public function definition() {
        ...
        $m->addHelpButton('shells', 'wdsprefs:split_how_many', 'block_wdsprefs');
        ...
        $m->addHelpButton('autopop', 'wdsprefs:split_autopop', 'block_wdsprefs');
...
class split_form_update extends split_form implements updating_form {
    ...
    public function definition() {
        ...
        $currentsplits = wds_split::in_course($course);

        $sections = $course->sections;

        $shells = wds_split::groups($currentsplits);
        ...
        foreach ($currentsplits as $split) {
            ...
            $display = "$course->department $course->course_number Section $section->section_number";
        ...
        if (!empty($sections) or count($course->sections) > 2) {
            ...
            $m->addHelpButton('reshelled', 'wdsprefs:split_how_many', 'block_wdsprefs');
...
class split_form_decide extends split_form {
    ...
    public function definition() {
        ...
        $this->prev = wds_split::exists($course) ? self::UPDATE : $this->prev;
        ...
        foreach ($sections as $section) {
            $before[$section->id] = "Section $section->section_number";
        }
    ...
    public function definition() {
        ...
        foreach (range(1, $this->_customdata['shells']) as $number) {
            ...
            foreach (explode(',', $values) as $sectionid) {
                $html .= '<li>Section ' . $sections[$sectionid]->section_number . '</li>';
            }
            $html .= '</ul>';
...
class split_form_finish implements finalized_form {
    ...
    public function process($data, $semesters) {
        ...
        $course = self::stupid_course($semesters, $semid, $couid);
        // $course = $semesters[$semid]->courses[$couid];
        // Checking to see if this has been split yet.
        $currentsplits = wds_split::in_course($course);
    ...
    public function save_or_update($data, $currentsplits) {
        ...
        foreach (range(1, $data->shells) as $grouping) {
            ...
            foreach (explode(',', $shellvalues) as $sectionid) {
                ...
                if (!$split = wds_split::get($splitparams)) {
                    $split = new wds_split();
    ...
    public function display() {
        ...
        echo $OUTPUT->continue_button(new moodle_url('/blocks/wdsprefs/splitview.php'));
    }
}