...
abstract class wds_preferences extends wds_external implements verifiable {
    ...
    public static function name() {
        $wanker = self::call('get_name');
        error_log("What is wanker: ". $wanker);
...
class wds_crosslist extends manifest_updater implements application, undoable {
    public $userid;
    public $sectionid;
    public $groupingid;
    public $shell_name;
    ...
    public function apply() {
        ...
        wds::inject_manifest(array($section));
    }

    public function unapply() {
        $sections = array($this->section());

        wds::inject_manifest($sections, function ($section) {
            $section->idnumber = '';
        });
    }
}
...
class wds_team_request extends wds_preferences implements application, undoable {
    public $semesterid;
    public $userid;
    public $courseid;
    public $requested;
    ...
    public function other_course() {
        if (empty($this->other_course)) {
            $course = wds_course::get(array('id' => $this->requested_course));
    ...
    public function other_user() {
        if (empty($this->other_user)) {
            $this->other_user = wds_user::get(array('id' => $this->requested));
        }
    ...
    public function course() {
        if (empty($this->course)) {
            $this->course = wds_course::get(array('id' => $this->courseid));
        }
    ...
    public function owner() {
        if (empty($this->owner)) {
            $this->owner = wds_user::get(array('id' => $this->userid));
        }
    ...
    public function semester() {
        if (empty($this->semester)) {
            $this->semester = wds_semester::get(array('id' => $this->semesterid));
        }
    ...
    public function label($from_userid = null) {
        ...
        $label = "$sem->year $sem->name$session $course->department $course->course_number";
    ...
    public function sections() {
        if (empty($this->sections)) {
            ...
            $this->sections = wds_team_section::get_all($params);
        }
    ...
    public function apply() {
        ...
        if ($this->approved()) {
            ...
            $url = new moodle_url('/blocks/wdsprefs/team_request.php');
...
class wds_team_section extends manifest_updater implements application, undoable {
    public $requesterid;
    public $courseid;
    public $sectionid;
    public $groupingid;
    ...
    public static function in_requests(array $requests) {
        ...
        foreach ($requests as $request) {
            $internal = self::get_all(wds::where()
                ->join('{enrol_wds_sections}', 'sec')->on('sectionid', 'id')
    ...
    public function request() {
        if (empty($this->request)) {
            $this->request = wds_team_request::get(array('id' => $this->requestid));
        }
    ...
    public function user() {
        if (empty($this->user)) {
            $this->user = wds_user::by_id($this->requesterid);
        }
    ...
    public function course() {
        if (empty($this->course)) {
            $this->course = wds_course::by_id($this->courseid);
        }
    ...
    public function unapply() {
        wds::inject_manifest(array($this->section()), function($sec) {
            $sec->idnumber = '';
        });
    }
}
