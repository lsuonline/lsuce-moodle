...
trait WDS_Enroll {
    public static function get_enrol_plugin() {
        
        include_once(dirname(__FILE__) . '/lib.php');
        $class = "enrol_wds_plugin";
        return new $class();
    }
}
...
abstract class wds {
    ...
    public static function require_dalos() {
        $dalo = self::base('classes/dalo');

        require_once($dalo . '/base.php');
        require_once($dalo . '/extern.php');
        require_once($dalo . '/lib.php');
        require_once($dalo . '/dalos.php');
        require_once($dalo . '/error.php');
        require_once($dalo . '/filter.php');
    }
    ...
    public static function where($field = null) {
        return new wds_dalo_filter($field);
    }
    ...
    public static function unenroll_users(array $sections, $silent = true) {
        ...
        $enrol->issilent = $silent;

        foreach ($sections as $section) {
            $section->wd_status = self::PENDING;
            // the parent table should be wds_sections.
            $section->save();
        }
    ...
    public static function enroll_users(array $sections, $silent = true) {
        ...
        foreach ($sections as $section) {
            ...
            $section->wd_status = self::PROCESSED;
    ...
    public static function reprocess_department($semester, $department, $silent = true) {
        ...
        wds_error::department($semester, $department)->handle($enrol);

        $ids = wds_section::ids_by_course_department($semester, $department);

        $pending = wds_section::get_all(self::where('id')->in($ids)->status->equal(self::PENDING));
        $processed = wds_section::get_all(self::where('id')->in($ids)->status->equal(self::PROCESSED));
    ...
    public static function reprocess_course($course, $silent = true) {
        $sections = wds_section::from_course($course, true);
    ...
    public static function reprocess_for($teacher, $silent = true) {
        ...
        if ($provider and $provider->supports_reverse_lookups()) {
            ...
            $semesters = wds_semester::in_session();

            foreach ($semesters as $semester) {
                $courses = $info->teacher_info($semester, $wdsuser);
    ...
    public static function drop_semester($semester, $report = false) {
        ...
        if (file_exists($CFG->dirroot.'/blocks/wdsprefs/classes/s_handler.php')) {
            require_once($CFG->dirroot.'/blocks/wdsprefs/classes/s_handler.php');
            blocks_wdsprefs_wds_handler::wds_semester_drop($semester);
        }
        if (file_exists($CFG->dirroot.'/blocks/post_grades/events.php')) {
            require_once($CFG->dirroot.'/blocks/post_grades/events.php');
            post_grades_handler::wds_semester_drop($semester);
        }

        if(!$semester->semester_ignore){ // LSU Enhancement if semester is ignored do not delete
            wds_semester::delete($semester->id);
        }
    ...
    public static function list_plugins() {
        ...
        foreach (scandir($basedir) as $file) {
            if (file_exists($basedir.DIRECTORY_SEPARATOR.$file.DIRECTORY_SEPARATOR.'provider.php')) {
                ...
                $data = $class::wds_list_provider($data);
            }
        }
        return $data->plugins;
    }
    ...
    public static function provider_class() {
        global $CFG;
        $providername = get_config('block_wdsprefs', 'enrollment_provider');
        ...
        if (file_exists($basedir.'/events.php')) {
            ...
            $fn    = 'wds_load_'.$providername.'_provider';
            $class::$fn($data);
        }
    ...
    public static function translate_error($e) {
        ...
        $a->pluginname =
            ...
            get_config('block_wdsprefs', 'enrollment_provider');
    ...
    public static function get_task_status_description() {
        ...
        $scheduledtask = \core\task\manager::get_scheduled_task('\block_wdsprefs\task\full_process');
