...
class block_quickmail_reminder_notification_course_grade_range_model_testcase extends advanced_testcase {
    ...
    public function test_notification_model_helper_supports_model() {
        ...
        $shortmodelclassname = notification_model_helper::get_model_class_name('course_grade_range');
        $this->assertEquals('course_grade_range_model', $shortmodelclassname);

        // Gets full model class name from type and key.
        $fullmodelclassname = notification_model_helper::get_full_model_class_name('reminder', 'course_grade_range');
        $this->assertEquals(course_grade_range_model::class, $fullmodelclassname);
        ...
        $conditionkeys = notification::get_required_conditions_for_type('reminder', 'course-grade-range');
        $this->assertCount(2, $conditionkeys);
        $this->assertContains('grade_greater_than', $conditionkeys);
        $this->assertContains('grade_less_than', $conditionkeys);
    }
