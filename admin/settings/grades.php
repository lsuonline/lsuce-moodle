...
if (has_capability('moodle/grade:manage', $systemcontext)
 or has_capability('moodle/grade:manageletters', $systemcontext)) { // speedup for non-admins, add all caps used on this page
    ...
    if ($ADMIN->fulltree) {
        $temp->add(new admin_setting_configcheckbox('grade_coursecateditable', get_string('coursecateditable', 'grades'), get_string('coursecateditable_help', 'grades'), 1));
        ...
        $temp->add(new admin_setting_configcheckbox('grade_droplow_limit',
            get_string('droplow_limit', 'grades'),
            get_string('droplow_limit_help', 'grades'), 0)
        );
    }
    $ADMIN->add('grades', $temp);
