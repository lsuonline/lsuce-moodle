...
if ($selected_course_id && ! in_array($selected_course_id, $userCourseIds)) {
    var_dump('no');
    die; // TODO - change this for reals.
} else {
...
// Commented out: require_capability('local/cas_help_links:editcategorysettings', $context);.
...
list($weeks
    , $userTotals
    , $clickTotals) = \local_cas_help_links_logger::get_teacher_current_semester_usage_data($user_id, $selected_course_id);
...
echo $output->single_select('user_analytics.php'
                            , 'course'
                            , \local_cas_help_links_utility::get_teacher_course_selection_array($user_id)
                            , $selected_course_id
                            , ['' => 'All my courses']
                            , 'course-select'
                            , []);
