...
class local_tcs_external extends external_api {
     ...
     * @return string welcome message
     */
    public static function tcsAjax($datachunk) {
        // error_log("\n tcsAjax() -> FRACK-YA");
        ...
        if (method_exists($tcsajax, $function)) {
            $CFG->local_tcs_logging ? error_log("\nAJAX.php => Success, now calling ".
                $function." from ".get_class($tcsajax).".php") : null;

            $left_over_data = call_user_func(array($tcsajax, $function), $params);
            $CFG->local_tcs_logging ? error_log("\nAJAX.php => Done\n") : null;
        } else {
        ...
        $params33 = [
            'data' => json_encode($left_over_data)
        ];
        return $params33;
    }
