...
class ArrayToXML
{
     ...
     * @return string XML
     */
    public static function toXml($data, $rootNodeName = 'data', $filename, &$xml=null)
    {
    ...
    foreach( $data as $key => $value ) {
        ...
        if ( is_array( $value ) ) {
            ...
            if ( ArrayToXML::is_assoc( $value ) || $numeric ) {
                ...
                {
                    $node = $xml->addChild( $key, null, 'Uleth Testing' );
                    if ($attrs) {
            ...
            // recrusive call.
            if ( $numeric ) $key = 'anon';
            ArrayToXML::toXml( $value, $key, $filename, $node );
        } else {

                // older SimplXMLElement Libraries do not have the addChild Method
                if (method_exists('SimpleXMLElement','addChild'))
                {
                    $childnode = $xml->addChild( $key, $value, 'Uleth Testing' );
                    if ($attrs) {
                        foreach($attrs as $key => $attribute) {
