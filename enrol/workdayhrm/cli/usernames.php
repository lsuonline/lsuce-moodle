...
// Require the main Workday HRM class.
require_once('../classes/workdayhrm.php');

// Set the webservice token.
$wstoken = lsuworkdayhrm::get_token();

// Get emails for pulling and populating Workday HRM info.
$users = itrulyhateworkdayhrm::get_emails();
...
foreach ($users as $user) {
    ...
    // Get the student info from the Workday HRM webservice.
    $userinfo = itrulyhateworkdayhrm::get_student_info($wstoken, $user);
      ...
      if (isset($userinfo->student[0])) {
        ...
        $update = itrulyhateworkdayhrm::fast_update_local($student, $user);

     }

}

class itrulyhateworkdayhrm {

  public static function get_student_info($token, $user) {
    // Get the data needed.
    $s = lsuworkdayhrm::get_workdayhrm_settings();
  ...
  public static function update_local($student, $user) {
    ...
    $dataobject = array('id'         => $user->id,
                        ...
                        'workdayhrm_id'      => isset($student->objectId) ? $student->objectId : null,
  ...
  public static function fast_update_local($student, $user) {
    ...
    $workdayhrmid    = isset($student->objectId) ?  $student->objectId : null;

    $sql = 'UPDATE mdl_scotty_enr s
            SET s.logonid = "' . $logonid . '",
                s.x_number = "' . $xnumber . '",
                s.workdayhrm_id = "' . $workdayhrmid . '",
                ' . $lsuid . ',
                s.updated = 1,
