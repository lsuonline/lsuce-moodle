...
Some details for when a user enrollment get's pushed from banner.

My Log:
--------------------------------------------------------
Enrolment-> 20158.201802 | XML ID:131484 | 
131484 updated in LMB Table |
recovering grades:enrolled:

process_file()
|___
    \/
    process_membership_tag(**THIS ONE**) || process_xml_line_error()
    |___
        \/
        process_membership_tag_error()
            |___
                \/
            process_person_membership_tag()
                "updated in LMB Table"
            |___
                \/
                process_enrolment_log()

Moodle Log:
--------------------------------------------------------
131484 inserted into LMB enrolments table |
recovering grades:enrolled: 
updated LMB enrolments Table |

process_xml_line()
|___
    \/
    process_xml_line_error()
    |___
        \/
        process_membership_tag_error()
        |___
...
class enrol_lmb_plugin extends enrol_plugin {
    ...
    public function cron() {
        ...
        // DALO - UofL - do digest email
        if ($this->get_config('emailerrorsto')) {
            //select all error_log entries that havent been sent, and create the body of the email from them.
            $body="";
            if ($errs=$DB->get_records('lmb_error_log', array('sent'=>0), 'timereceived ASC')) {
                foreach ($errs as $err) {
                    $body.=date('[Y-m-d H:i:s] ', $err->timereceived)." ".$err->error_text."\r\n\r\n";
                }
                print "...sending error email...";
                mail($this->get_config('emailerrorsto'), "Error in LMB: ".$CFG->wwwroot, "From ".$CFG->wwwroot."\r\n\r\n" .  $body. "\n");
            }
            //now set records as 'sent'
            $sql= 'UPDATE '.$CFG->prefix.'lmb_error_log SET sent=1 WHERE sent=0';
            $DB->execute($sql);
        }
    ...
    public function process_file($filename = null, $force = false, $folderprocess = false, $processid = null) {
        ...
        $mdl_user_columns = $DB->get_columns("user", false);
        //$row_count2 = $DB->get_columns("user");
        if (isset($mdl_user_columns['uofl_id'])) {
            $this->log_line("There is a uofl column");
        } else {

            $this->log_line("No uofl column exists, creating one now.");
            $row_count1 = $DB->execute("ALTER TABLE mdl_user ADD uofl_id VARCHAR NULL DEFAULT '0'");
            $row_count2 = $DB->execute("ALTER TABLE mdl_enrol_lmb_people ADD uofl_id VARCHAR NULL DEFAULT '0'");
            $this->log_line("has the uofl column been created!");
        }
    ...
    public function process_group_tag($tagcontents) {
        if (preg_match('{<group>.*?<grouptype>.*?<typevalue.*?\>(.+?)</typevalue>.*?</grouptype>.*?</group>}is',
            ...
            error_log("Error processing group");
        }
    }
    ...
    public function process_course_section_tag($tagcontents) {
        ...
        if (isset($CFG->local_lmb_debugging_xml) && $CFG->local_lmb_debugging_xml == 1) {
            error_log("\nLMB -> process_course_section_tag () -> START");
        }

        if (isset($CFG->local_lmb_debugging_xml) && $CFG->local_lmb_debugging_xml == 1) {
            error_log("\nLMB -> process_course_section_tag () -> XML: ". print_r($tagcontents, 1));
        }
        ...
        $start_year = $this->get_config('uofl_course_year');
        $start_term = $this->get_config('uofl_course_term');
        $settings_term = null;
        ...
        if (preg_match('{<sourcedid>.*?<id>(.+?)</id>.*?</sourcedid>}is', $tagcontents, $matches)) {
            ...
            $logline .= 'CID:'.$course->sourcedid.' |';
        } else {
            $logline .= $logline." CID: not found |";
            return false;
        }
        if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
            $logline .= "\nLMB -> process_course_section_tag () -> is the course->term set: ". $course->term . "\n";
        }
        ...
        if (preg_match('{<timeframe>.*?<begin.*?\>(.+?)</begin>.*?</timeframe>}is', $tagcontents, $matches)) {
            ...
            if ($date[0] != $start_year) {
                $logline .= " this instance year is: ".$start_year." course year sent over wire is: ".$date[0]." |";
                return false;
            }

            // date[1] is month
            // date[2] is day
            // if (($start_min <= $start_term) && ($start_term <= $start_max)) {

            $settings_term = $start_year . "0" . $start_term;

            if (isset($course->term)) {
                if ($course->term != $settings_term) {

                    if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                        $logline .= "\nLMB -> process_course_section_tag () -> course_term != settings_term\n";
                    }

                    $logline .= " Rejected course ".$course->num." in ".$course->dept." section ".$course->section.", has this term in XML: " . $course->term . " and our term settings is set to: " . $settings_term . " |";
                    $this->log_line($logline);
                    return false;
                } else {
                    if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                        $logline .= "\nLMB -> process_course_section_tag () -> course_term == settings_term, continuing...\n";
                    }
                }
            } else {

                if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                    $logline .= "\nLMB -> process_course_section_tag () -> course_term is NOT SET\n";
                }
            }
        ...
        if (preg_match('{<org>.*?<orgunit>(.+?)</orgunit>.*?</org>}is', $tagcontents, $matches)) {
            ...
            $course->depttitle = '';
            $logline .= ' This course has no title |';
        }

        $logline .= " ".$course->longtitle." |";
        ...
        if ($oldcourse = $DB->get_record('enrol_lmb_courses', array('sourcedid' => $course->sourcedid))) {
            ...
            if (enrol_lmb_compare_objects($course, $oldcourse)) {
                if ($DB->update_record('enrol_lmb_courses', $course)) {
                    $logline .= ' updated course in LMB Courses |';
                } else {
                    $logline .= ' failed to update course in LMB table |';
                    $status = false;
                }
            } else {
                $logline .= ' course found in LMB table, no updates needed |';
            }
        } else {
            if ($course->id = $DB->insert_record('enrol_lmb_courses', $course, true)) {
                $logline .= ' updated course in LMB Courses  |';
            } else {
                $logline .= ' failed to update course in LMB Courses |';
                $status = false;
            }
        }
        ...
        if ($status && ($currentcourse = $DB->get_record('course', array('idnumber' => $moodlecourse->idnumber)))) {
            $logline .= "course is found in course table|";
            ...
            if ($update) {
                ...
                // $logline .= " updated course:");
                $logline .= " updated course in course Table |";

            } else {
                $logline .= " No updates needed for course Table |";
            ...
            //$logline .= ' attempting to create new course';
            $this->create_shell_course(
                $course->sourcedid,
                enrol_lmb_expand_course_title($course, $this->get_config('coursetitle')),
                enrol_lmb_expand_course_title($course, $this->get_config('courseshorttitle')),
                $cat->id,
                $logline,
                $status,
                false,
            ...
            //$logline .= ' new course has been added to course table';
        }
        ...
        if ($status) {
            ...
            if (!$tmpstatus) {
                $logline .= " ERROR trying to restore users to course |";
            }
        }
    ...
    public function create_shell_course($idnumber, $name, $shortname, $catid, &$logline, &$status,
        ...
        if ($this->get_config('usemoodlecoursesettings') && ($moodlecourseconfig)) {
            ...
            $moodlecourse->format               = 'topcoll';
            $moodlecourse->numsections          = 14;
        ...
        // Setup the blocks for the TEMPLATE COURSE

        if ($this->get_config('uofl_left_inst') == "") {
            $left_text = array();
        } else {
            $left_text = explode(",", $this->get_config('uofl_left_inst'));
        }

        if ($this->get_config('uofl_right_inst') == "") {
            $right_text = array();
        } else {
            $right_text = explode(",", $this->get_config('uofl_right_inst'));
        }


        $default_left_block = array();
        $default_right_block =  array('activity_modules',  'news_items', 'calendar_month','calendar_upcoming');

        $new_left_block = array_merge($default_left_block, $left_text);
        $new_right_block = array_merge($default_right_block, $right_text);

        $sample_block = array('activity_modules',  'news_items', 'calendar_month','calendar_upcoming');
        ...
        $page->set_course($moodlecourse);
        $blocknames = array(
        BLOCK_POS_LEFT => $new_left_block,
        BLOCK_POS_RIGHT =>$new_right_block);
    ...
    /**
     * 
     * @param string - this value is what is missing so check user db using other variables
     */
    public function perform_extra_check($student, $check_this){
        global $DB;

        if ($check_this == "bid") {
            $select = "username = '".$student->username."'";
            $table = 'enrol_lmb_people';
            $person = $DB->get_records_select($table,$select, null, null);
            
            $what_is_this = $person[0]->sourcedid;
            $what_is_this2 = $person[18215]->sourcedid;
            $student_index = null;
            foreach ($person as $pos => $value) {
                $what_is_val = $value;
                $what_is_key = $pos;
            ...
            if (isset($person[$student_index]->sourcedid)) {
                return $person[$student_index]->sourcedid;
            } else {
                return 0;
            }
        } elseif ($check_this == "username") {
            //$person = $DB->get_record('enrol_lmb_people', array('idnumber' => $student->sourcedid));
            $select = "idnumber = '".$student->sourcedid."'";
            $table = 'enrol_lmb_people';
            $person = $DB->get_records_select($table, $select, null, null);
    ...
    public function process_person_tag($tagcontents) {
        ...
        if (isset($CFG->local_lmb_debugging_xml) && $CFG->local_lmb_debugging_xml == 1) {
            error_log("\nLMB -> process_person_tag () -> START");
        }

        if (isset($CFG->local_lmb_debugging_xml) && $CFG->local_lmb_debugging_xml == 1) {
            error_log("\nLMB -> process_person_tag () -> XML: ". print_r($tagcontents, 1));
        }
        ...
        if (preg_match('{<userid.+?useridtype *= *"Email ID".*?\>(.+?)</userid>}is', $tagcontents, $matches)) {
            $person->email = trim($matches[1])."@uleth.ca";
        ...
        if (preg_match('{<userid.+?useridtype *= *"SCTID".*?\>(.+?)</userid>}is', $tagcontents, $matches)) {
            ...
            $person->uofl_id = trim($matches[1]);

            if (isset($CFG->local_lmb_debugging_xml) && $CFG->local_lmb_debugging_xml == 1) {
                error_log("\nprocess_person_tag() -> What is uofl_id: ". $person->uofl_id);
            }
            $logline .= ' UofL_ID:'.$person->uofl_id.' |';
        }


        // Gender.
        if (preg_match('{<demographics>.*?<gender>(.+?)</gender>.*?</demographics>}is', $tagcontents, $matches)) {
            $person->gender = trim($matches[1]);
        }

        // Role type
        if (preg_match('{<institutionrole.+?institutionroletype *= *"(.*?)"/>}is', $tagcontents, $matches)) {
            $person->role_type = trim($matches[1]);
        }

        //        $role_exp_temp = '{<institutionrole.+?institutionrole *= *"SCTID".+?password *= *"(.*?)">.*?</userid>}is';
        //        if (preg_match($role_exp_temp, $tagcontents, $matches)) {
        //            $person->password = trim($matches[1]);
        //        }
        //         Email.
        if (preg_match('{<email>(.*?)</email>}is', $tagcontents, $matches)) {
            $person->diff_email = trim($matches[1]);
        }


        if ($run_check == 1) {
            if (!$person->sourcedid = $this->perform_extra_check($person, "bid")) {
                $this->log_line(' XML ID not found |');
                return false;
            }
        }
        ...
        // }
        // DALO - UofL - this is from our old LMB
        // Academice major
        ...
        switch ($this->get_config('usernamesource')) {
            ...
            default:
                $status = false;
                $logline .= ' Error-P1 |';
                // $this->enrol_lmb_store_error('Person: Bad enrol_lmb_usernamesource setting. Set to: '.$config->usernamesource.'.'); // DALO

        }

        if ($this->get_config('sourcedidfallback') && isset($person->username) && trim($person->username)=='') {
        ...
        if (!isset($person->username) || (trim($person->username)=='')) {
            ...
            // $logline .= '| User('.$person->sourcedid.') had no username in xml, their role is:'.$person->role_type.'|';
            // $this->enrol_lmb_store_error('Person: no username');   // DALO
        }
        ...
        if (isset($person->fullname)) {
            // UofL - Use the nickname instead of the fullname
            // $lmbperson->fullname = $person->nickname;
        ...
        if (isset($person->uofl_id)) {
            $lmbperson->uofl_id = $person->uofl_id;
        }
        ...
        if (debugging()) {
            // A way to see the XML for specific user(s) using their email
            if (isset($CFG->local_lmb_debugging_xml_custom) && $CFG->local_lmb_debugging_xml_custom != "") {
                $peeper_list = explode(',', $CFG->local_lmb_debugging_xml_custom);
                foreach ($peeper_list as $peeper) {
                    if ($peeper == $lmbperson->email) {
                        error_log("\n\n---------------------------------------------------------------\n");
                        error_log("LMB -> User Watch email -> XML: ". print_r($tagcontents, 1));
                        error_log("\n---------------------------------------------------------------\n\n");
                        break;
                    }
                }
            }
            // A way to see the XML for specific user(s) using their sourcedid
            if (isset($CFG->local_lmb_debugging_xml_custom_sourcedid) && $CFG->local_lmb_debugging_xml_custom_sourcedid != "") {
                $peeper_list2 = explode(',', $CFG->local_lmb_debugging_xml_custom_sourcedid);
                foreach ($peeper_list2 as $peeper2) {
                    if ($peeper2 == $lmbperson->sourcedid) {
                        error_log("\n\n---------------------------------------------------------------\n");
                        error_log("LMB -> User Watch sourcedid-> XML: ". print_r($tagcontents, 1));
                        error_log("\n---------------------------------------------------------------\n\n");                        
                        break;
                    }
                }
        ...
        if ($oldlmbperson = $DB->get_record('enrol_lmb_people', array('sourcedid' => $lmbperson->sourcedid))) {
            ...
            if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                $logline .= "\nLMB -> process_person_tag () -> found user in enrol_lmb_people table: ". print_r($oldlmbperson, 1). "\n";
            }
            if (enrol_lmb_compare_objects($lmbpersonslash, $oldlmbperson)) {
                if (!$DB->update_record('enrol_lmb_people', $lmbpersonslash)) {
                    $logline .= " NOTICE -> User already exists in enrol_lmb_people, update aborted |";
                    $status = false;
                    // $this->enrol_lmb_store_error('Person: error updating lmb_people.');
                } else {
                    // $logline .= '| updated lmb table:';
                    $logline .= ' '.$lmbpersonslash->uofl_id.' updated in enrol_lmb_people |';
                }
            } else {
                $logline .= ' '.$lmbpersonslash->uofl_id.' already in enrol_lmb_people |';
                // $logline .= '| no lmb changes to make:';
            }
        } else {
            if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                $logline .= "\nLMB -> process_person_tag () -> user NOT found, going to insert \n";
            }
            if (!$DB->insert_record('enrol_lmb_people', $lmbpersonslash)) {
                // $logline .= 'error inserting enrol_lmb_people:';
                if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                    $logline .= "\nLMB -> process_person_tag () -> ERROR-> user INSERT FAILED \n";
                }
                $logline .= " Error-P4 |";
                // $logline .= '| ERROR inserting user: '.$lmbpersonslash->id.' '.$lmbpersonslash->fullname.' in enrol_lmb_people code(1)';
                $status = false;
                // $this->enrol_lmb_store_error('Person: error inserting lmb_people.');
            } else {
                if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                    $logline .= "\nLMB -> process_person_tag () -> Success-> user has been added to enrol_lmb_people.\n";
                }
                $logline .= ' inserted into enrol_lmb_people |';
            }
        }

        if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
            $logline .= "\nLMB -> process_person_tag () -> lmbperson obj: ". print_r($lmbperson, 1). "\n";
        }

        if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
            $logline .= "\nLMB -> process_person_tag () -> person obj: ". print_r($person, 1). "\n";
        }
        ...
        if ($this->get_config('createusersemaildomain')) {
            if (isset($lmbperson->email) && ($lmbperson->email) && ($domain = explode('@', $lmbperson->email))
                    && (count($domain) > 1)) {
                // print in extra debugging........
                if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                    $logline .= "\nLMB -> process_person_tag () -> what is the email: ". $lmbperson->email. "\n";
                }
                ...
                if (isset($person->email) && ($person->email) && ($domain = explode('@', $person->email))
                    && (count($domain) > 1)) {
                // print in extra debugging........
                    if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                        $logline .= "\nLMB -> process_person_tag () -> what is the email: ". $person->email. "\n";
                    }
                } else {
                    if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                        $logline .= "\nLMB -> process_person_tag () -> There is no lmbperson email or person email, could be staff?\n";
                    }
                }

                if (isset($person->role_type) && $person->role_type == "Staff") {
                    $logline .= ' User is Staff |';
                } elseif (isset($person->role_type) && $person->role_type == "Faculty") {
                    $logline .= ' User is Faculty |';
                } else {
                    if (isset($person->diff_email)) {
                        $logline .= ' No UofL email set but has alt email: '.$person->diff_email.' |';
                    } else {
                        if (isset($person->role_type) && $person->role_type == "Student" || $person->role_type == "ProspectiveStudent") {
                            $logline .= ' This '.$person->role_type.' has no email address |';
                        } else {
                            $logline .= ' Error-P6 |';
                        }
                    }
        ...
        if (isset($lmbperson->email)) {
            ...
            if ($emailallow && $lmbperson->recstatus != 3 && trim($lmbperson->username) != '') {
                ...
                if (($oldmoodleuser = $DB->get_record('user', array('idnumber' => $moodleuser->idnumber)))
                    ...
                    if ($oldmoodleuser->uofl_id == 0) {
                        $the_u_id = (int)$lmbperson->uofl_id;
                        $add_u_id = new stdClass();
                        $add_u_id->id = $moodleuser->id;
                        $add_u_id->uofl_id = $the_u_id;
                        $cleaned = array();
                        $cleaned['id'] = $moodleuser->id;
                        $cleaned['uofl_id'] = $the_u_id;
                        
                        // $DB->update_record('user', $add_u_id); 
                        $DB->update_record_raw('user', $cleaned);
                        $logline .= $the_u_id.' updated UofL ID in user table |';
                    ...
                    if (enrol_lmb_compare_objects($moodleuser, $oldmoodleuser) || ($this->get_config('customfield1mapping')
                        ...
                        if ((strcasecmp($oldmoodleuser->username, $moodleuser->username) !== 0)
                            ...
                            $logline .= ' Error-P7 |'; // | ERROR username collision while trying to update: '.$moodleuser->id.' '.$moodleuser->fullname.' code(2)';
                            $status = false;
                            // $this->enrol_lmb_store_error('Person: username collision while trying to update. Username: '. $moodleuser->username);

                        } else {
                            if ($DB->update_record('user', $moodleuser)) {
                                if (isset($moodleuser->uofl_id)) {
                                    $logline .= ' '.$moodleuser->uofl_id.' was updated in mdl_user |';
                                }
                        ...
                        if (isset($moodleuser->uofl_id)) {
                            $logline .= ' '.$moodleuser->uofl_id.' already in mdl_user table |';
                        }
                        // $logline .= '| no changes to make:';
                    }
                } else {
                    ...
                    if ($this->get_config('createnewusers')) {
                        if ($collisionid = $DB->get_field('user', 'id', array('username' => $moodleuser->username))) {
                            ...
                            $logline .= ' WARNING username collision has occurred (user row id: '. $collisionid . '), going to merge: '. $moodleuser->username.' |';
                            if (isset($lmbperson->uofl_id) && isset($lmbperson->sourcedid)) {

                                $temp_user_obj = new stdClass;
                                $temp_user_obj->id = $collisionid;
                                $temp_user_obj->idnumber = $lmbperson->sourcedid;
                                $temp_user_obj->uofl_id = $lmbperson->uofl_id;

                                if (isset($CFG->local_lmb_user_force_merge) && $CFG->local_lmb_user_force_merge) {

                                    if ($DB->update_record('user', $temp_user_obj)) {                                    
                                        $logline .= ' Merge was a success |';
                                    } else {
                                        $logline .= ' ERROR, merge failed horribly...... code(3.2)  |';
                                    }

                                    $status = $status && $this->restore_user_enrolments($lmbperson->sourcedid);

                                } else {
                                    $logline .= ' WARNING Merge setting is not set in LMB Settings |';
                                    return false;
                                }

                            } else {
                                $logline .= ' ERROR, either uofl id and sourcedid were NOT FOUND code(3.1)  |';
                                $status = false;
                            }

                        } else {
                            if ($lmbperson->uofl_id) {
                                $moodleuser->uofl_id = (int)$lmbperson->uofl_id;
                            }

                            if ($id = $DB->insert_record('user', $moodleuser, true)) {

                                $cleaned = array();
                                $cleaned['id'] = $id; //  $moodleuser->id;
                                $cleaned['uofl_id'] = $lmbperson->uofl_id;
                                $DB->update_record_raw('user', $cleaned);

                                $logline .= ' user created in user table |';
                                if (isset($lmbperson->customfield1)) {
                                    $this->update_custom_mapping(
                                        $id,
                                        $lmbperson->customfield1,
                                        $this->get_config('customfield1mapping')
                                    );
                                }
                                $moodleuser->id = $id;
                                ...
                                $logline .= ' ERROR failed to insert new user: '.$moodleuser->id.' '.$moodleuser->fullname.' code(3) |';
            ...
            /*
             * if(isset($person->role_type) && $person->role_type == "Staff"){
                $logline .= 'User is Staff, no email address found|';
            }elseif(isset($person->role_type) && $person->role_type == "Faculty"){
                $logline .= 'User is Faculty, no email address found|';
            }else{
                if(isset($person->diff_email)){
                    $logline .= 'User has no UofL email: '.$person->diff_email.'|';
                }else{
                    $logline .= 'no email address found|';
                }
            }
            */
            if (isset($lmbperson->sourcedid)) {
                $logline .= ' no email address found for BID: ' . $lmbperson->sourcedid . ' |';
            } else if (isset($person->sourcedidsource)) {
                $logline .= ' no email address found for BID: ' . $person->sourcedidsource . ' |';
            } else {
                $logline .= ' no email address found or Souredid. |';
            }
    ...
    public function process_term_tag($tagcontents) {
        ...
        if ($oldterm = $DB->get_record('enrol_lmb_terms', array('sourcedid' => $term->sourcedid))) {
            ...
            if ($id = $DB->update_record('enrol_lmb_terms', $term)) {
                $logline .= ' updated term in LMB table |';
            } else {
                $logline .= ' failed to update term in LMB table |';
                $status = false;
            }
        } else {
            if ($id = $DB->insert_record('enrol_lmb_terms', $term, true)) {
                $logline .= ' created term in LMB table |';
                $term->id = $id;
            } else {
                $logline .= ' failed to insert term in LMB table |';
                $status = false;
            }
        }
    ...
    public function process_person_membership_tag($tagcontents) {
        ...
        if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
            error_log("\n\n---------------------------------------------------------------\nprocess_person_membership() -> START\n\n");
        }
        ...
        if (isset($CFG->local_lmb_debugging_xml) && $CFG->local_lmb_debugging_xml == 1) {
            error_log("\nLMB -> process_person_membership_tag () -> here is the contents: ". print_r($tagcontents, 1));
        } 
        ...
        if (preg_match('{<sourcedid>.*?<id>(.+?)</id>.*?</sourcedid>}is', $tagcontents, $matches)) {
            ...
            if (preg_match('{.....\.(.+?)$}is', $enrolment->coursesourcedid, $matches)) {
                ...
                $funny_parts = explode('.', $enrolment->term);
                $temp_course_term = $funny_parts[0];
                // $temp_course_term = $funny_parts[1];

                // if the term is not the same as this instance, let's just abort now.
                $settings_term = $start_year . "0" . $start_term;
                if (isset($temp_course_term)) {
                    if ($temp_course_term != $settings_term) {
                        
                        if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                            error_log("\nLMB -> process_person_membership_tag () -> course_term != settings_term");
                            $logline .= " Skipping enrollment, wrong term. XML has this term: ". $temp_course_term . " and our term settings is set to: " . $settings_term . " |";
                        }
                        ...
                        if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                            $logline .= "\nLMB -> process_person_membership_tag () -> course_term does NOT settings_term, continuing...\n";
                        }
                    }
                }
            ...
            $logline .= ' '.$enrolment->coursesourcedid.' |';
        } else {
            $logline .= ' CID not found |';
            if (isset($CFG->local_lmb_debugging_xml) && $CFG->local_lmb_debugging_xml == 1) {
                error_log("\n LMB -> process_person_membership_tag () -> Status set to false CP - 1");
            }
            $status = false;
        }
        ...
        if (preg_match('{<member>(.*?)</member>}is', $tagcontents, $membermatches)) {
            ...
            if (preg_match('{<sourcedid>.*?<id>(.+?)</id>.*?</sourcedid>}is', $member, $matches)) {
                ...
                $logline .= ' XML ID:'.$enrolment->personsourcedid.' |';
            } else {
                $logline .= ' XML ID not found |';
                if (isset($CFG->local_lmb_debugging_xml) && $CFG->local_lmb_debugging_xml == 1) {
                    error_log("\n LMB -> process_person_membership_tag () -> Status set to false CP - 2");
                }
                $status = false;
            }
            ...
            if (preg_match('{<role.*?roletype *= *"(.*?)".*?\>.*?</role>}is', $member, $matches)) {
                ...
                // $logline .= 'U_Role not found:';
                if (isset($CFG->local_lmb_debugging_xml) && $CFG->local_lmb_debugging_xml == 1) {
                    error_log("\n LMB -> process_person_membership_tag () -> Status set to false CP - 3");
                }
                $status = false;
            }
            ...
            if ($recstatus == 3) {
                $enrolment->status = 0;
                if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                    error_log("\n\nLMB -> process_person_membership_tag () -> recstatus is 3 so setting enrol status to 0");
                }
            }
            ...
            if (preg_match('{<role.*?\>.*?<status>(.+?)</status>.*?</role>}is', $member, $matches)) {
                ...
                if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                    error_log("\n\nLMB -> process_person_membership_tag () -> preg match, pulled from xml status is: ". $enrolment->status);
                }
            } else {
                // $logline .= 'U_Status not found|';
                if (isset($CFG->local_lmb_debugging_xml) && $CFG->local_lmb_debugging_xml == 1) {
                    error_log("\n LMB -> process_person_membership_tag () -> Status set to false CP - 4");
                }
                $status = false;
            }
            ...
            if (isset($CFG->local_lmb_debugging_xml) && $CFG->local_lmb_debugging_xml == 1) {
                error_log("\n LMB -> process_person_membership_tag () -> Status set to false CP - 5");
            }
            $status = false;
        }
        ...
        if ($status) {
            ...
            if ($oldenrolment = $DB->get_record('enrol_lmb_enrolments', $params)) {
                ...
                // ==========================================================================================
                // error_log("\n\nprocess_person_membership_tag() -> printing old and new to file");
                // $enrol_lmb_compare_objects_file = '/Users/davidlowe/Desktop/Moodle_Banner_Mismatch/lmb_data.txt';
                // $two_plus_two = print_r($enrolment, 1) . "\n-------------------\n" . print_r($oldenrolment, 1);
                // file_put_contents($enrol_lmb_compare_objects_file, $two_plus_two);
                // ==========================================================================================
                ...
                if (enrol_lmb_compare_objects($enrolment, $oldenrolment)) {
                    if (!$DB->update_record('enrol_lmb_enrolments', $enrolment)) {
                        ...
                        $logline .= ' '.$enrolment->personsourcedid.' updated in LMB Table |';
                    }
                } else {
                    $logline .= ' '.$enrolment->personsourcedid.' already in LMB |';
                }
                ...
                if (!$enrolment->id = $DB->insert_record('enrol_lmb_enrolments', $enrolment, true)) {
                    $logline .= ' ERROR-E2 |';
                    if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                        error_log("\n LMB -> process_person_membership_tag () -> Status set to false CP - 6");
                        error_log("\nDB->insert_record for enrol_lmb_enrolments FAILED, enrolment is: ". print_r($enrolment, 1));
                    }
                    $status = false;
                } else {
                    $logline .= ' '.$enrolment->personsourcedid.' inserted into LMB enrolments table |';
                    // $logline .= ' success,inserted enrolment into lmb enrolments: ';
                }
            }
        }

        $status = $status && $this->process_enrolment_log($enrolment, $logline);
        $this_val = $this->get_config('log_enrollment_errors');
        
        if ($this_val == 1) {
            $this->log_line($logline);
        }
        if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
            error_log("\n\n---------------------------------------------------------------\nprocess_person_membership()-> END\n\n");
        }
        return $status;
    }
    ...
    public function process_enrolment_log($enrol, &$logline) {
        ...
        if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
            error_log("\n\n-----------------------\nprocess_enrolment_log() (PEL)-> START\n\n");
            error_log("\n\nPEL() -> enrol obj: ". print_r($enrol, 1));
        }
        ...
        if ($newcoursedid) {
            if ($userid = $DB->get_field('user', 'id', array('idnumber' => $enrol->personsourcedid))) {
                ...
                $logline .= ' this user does not have a UofL ID or email |';
                // store the user in the table
                include_once('tools/DeadEnrollments.php');

                $dead_humanoid = new DeadEnrollments($enrol);
                
                $dead_result = $dead_humanoid->addUser();

                if ($dead_result) {
                    $logline .= ' user was added to list of dead enrollments |';
                } else {
                    $logline .= ' user is already in dead enrollments for this course |';
                }

                $status = false;
            }
        } else {
            $logline .= ' course '.$enrol->coursesourcedid.' was not found |';
            $status = false;
        }
        ...
        if (enrol_lmb_compare_objects($enrolup, $enrol)) {
            if (!$DB->update_record('enrol_lmb_enrolments', $enrolup)) {
                $logline .= ' error updating LMB enrolments Table |';
                $status = false;
            } else {
                $logline .= ' updated LMB enrolments Table |';
            }
        }

        unset($enrolup);
        if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
            error_log("\n\n-----------------------\nprocess_enrolment_log() -> END\n\n");
        }
        return $status;
        ...
        if ($instance = $this->get_instance($courseid)) {
            ...
            if (isset($CFG->local_lmb_debugging) && $CFG->local_lmb_debugging == 1) {
                ...
                if ($ue = $DB->get_record('user_enrolments', array('enrolid'=>$instance->id, 'userid'=>$userid))) {
                    $logline .= "\nlmb_assign_role_log() -> SUCCESS, user has been enrolled.\n";
                } else {

                    $logline .= "\nlmb_assign_role_log() -> FAIL, user has not been enrolled.\n";
                    $logline .= "\nlmb_assign_role_log() -> FAIL here are the vars passed in: \n";
                    $logline .= "\nlmb_assign_role_log() -> FAIL instance: ". print_r($instance, 1). "\n";
                    $logline .= "\nlmb_assign_role_log() -> FAIL userid: ". print_r($userid, 1). "\n";
                    $logline .= "\nlmb_assign_role_log() -> FAIL roleid: ". print_r($roleid, 1). "\n";
                    $logline .= "\nlmb_assign_role_log() -> FAIL restrictstart: ". print_r($restrictstart, 1). "\n";
                    $logline .= "\nlmb_assign_role_log() -> FAIL restrictend: ". print_r($restrictend, 1). "\n";
                    $logline .= "\nlmb_assign_role_log() -> FAIL userstatus: ". print_r($userstatus, 1). "\n";
                    $logline .= "\nlmb_assign_role_log() -> FAIL recover: ". print_r($recover, 1). "\n";
                }
            } else {
                $logline .= 'User has been enrolled.';
            }
