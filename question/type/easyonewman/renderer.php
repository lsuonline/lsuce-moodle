...
class qtype_easyonewman_renderer extends qtype_renderer {
    public function formulation_and_controls(question_attempt $qa,
        ...
        else //when user attempting answer
        {
            $this->page->requires->js_init_call('M.qtype_easyonewman.insert_easyonewman_answer_pagenavigation', array($qa->get_slot(),$moodleroot,$stagoreclip, $qa->get_usage_id()));
        }
        ...
        if ($options->readonly) {
            if ($stagoreclip == 0) {  // Staggered.
                ...
                $result .= html_writer::div('', 'dropablediv'.$qa->get_slot().' flipable', array('id' => 'pos0'.$qa->get_slot()));
                $result .= html_writer::div('', 'dropablediv'.$qa->get_slot(), array('id' => 'pos1'.$qa->get_slot()));
                $result .= html_writer::div('', 'dropablediv'.$qa->get_slot(), array('id' => 'pos2'.$qa->get_slot()));
                $result .= html_writer::div('', 'dropablediv'.$qa->get_slot(), array('id' => 'pos3'.$qa->get_slot()));
                $result .= html_writer::div('', 'dropablediv'.$qa->get_slot(), array('id' => 'pos4'.$qa->get_slot()));
                $result .= html_writer::div('', 'dropablediv'.$qa->get_slot().' flipable', array('id' => 'pos5'.$qa->get_slot()));
                ...
                $result .= html_writer::div('', 'dropablediv'.$qa->get_slot(), array('id' => 'epos0'.$qa->get_slot()));
                $result .= html_writer::div('', 'dropablediv'.$qa->get_slot().' flipable', array('id' => 'epos1'.$qa->get_slot()));
                $result .= html_writer::div('', 'dropablediv'.$qa->get_slot(), array('id' => 'epos2'.$qa->get_slot()));
                $result .= html_writer::div('', 'dropablediv'.$qa->get_slot(), array('id' => 'epos3'.$qa->get_slot()));
                $result .= html_writer::div('', 'dropablediv'.$qa->get_slot().' flipable', array('id' => 'epos4'.$qa->get_slot()));
                $result .= html_writer::div('', 'dropablediv'.$qa->get_slot().' flipable', array('id' => 'epos5'.$qa->get_slot()));
                $result .= html_writer::end_tag('div');  // End divnew!
                $result .= html_writer::empty_tag('br', array('class' => 'cleared'));
            }
        } else {
            ...
            // UofL Fix ------------------
            // error_log("\n\nNEWMAN -> renderer.php");
            $temp = str_replace("question/type/easyonewman/pix", $CFG->wwwroot. "/question/type/easyonewman/pix", $temp);
    ...
    protected function require_js(question_attempt $qa, $readonly, $correctness) {
        ...
        $topnode = 'div.que.easyonewman#question-'.$qa->get_usage_id(). '-'. $qa->get_slot();
        if ($correctness) {
