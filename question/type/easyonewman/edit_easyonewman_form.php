...
class qtype_easyonewman_edit_form extends qtype_shortanswer_edit_form {
    ...
    protected function definition_inner($mform) {
        ...
        if (isset($question->id)) {
            $record = $DB->get_records('question_easyonewman', array('question' => $question->id ));
            $record_temp = array_values($record);
            $record = $record_temp[0];
        ...
        if ($stagoreclip == 1) {
            ...
            // UofL Fix ------------------
            // error_log("\n\nNEWMAN -> edit_easyonewman_form");
            $result .= file_get_contents('type/easyonewman/newman_dragable.html');
            $result = str_replace("question/type/easyonewman/pix", $CFG->wwwroot. "/question/type/easyonewman/pix", $result);
        ...
        // Add in the hidden inputs to hold answers.
            ...
            $PAGE->requires->js_init_call('M.qtype_easyonewman.init_getanswerstring', array($stagoreclip), false, $jsmodule);
