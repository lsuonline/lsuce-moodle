...
class qtype_easyomechjs_renderer extends qtype_renderer {
    public function formulation_and_controls(question_attempt $qa, question_display_options $options) {
        ...
        $uniqid = $qa->get_slot(); //uniqid(); Replacing uniqid with slotno as it will be always unique.
        $usageid = $qa->get_usage_id(); // Replacing uniqid with slotno as it will be always unique.
        ...
        $PAGE->requires->js(new moodle_url($CFG->wwwroot. $marvinjspath . '/js/marvinjslauncher.js'));
        ...
        if ($options->readonly) {
            ...
            $result .= html_writer::tag('textarea', $currentanswer, array(   // Removed unnecessary method call. 
                'id' => $myanswerid,
                'name' => $myanswerid,
                'style' => 'display:none;'
            ));
        ...
        $this->require_js($toreplaceid, $qa, $options->readonly, $options->correctness, $uniqid, $usageid);
    ...
    */
    protected function require_js($toreplaceid, question_attempt $qa, $readonly, $correctness, $uniqid, $usageid) {
        global $PAGE, $CFG;
        $marvinjsconfig  = get_config('qtype_easyomechjs_options');
        $marvinjspath    = $marvinjsconfig->path;
        // Uleth - DALO - this broke as of 3.7. The div id changed a bit.
        // $topnode        = 'div.que.easyomechjs#q' .$uniqid; // $qa->get_slot(); No need to call get slot method again. 
        $topnode        = 'div.que.easyomechjs#question-'. $usageid. "-". $uniqid; // $qa->get_slot(); No need to call get slot method again. 
        $feedbackimage  = '';
        if ($correctness) {
