...
M.qtype_jme = {
    ...
    insert_applet : function(Y, toreplaceid, appletid, name, topnode,
                                                                    appleturl, feedback, readonly, width, height, appletoptions){
        ...
        this.show_java(toreplaceid, appletid, name, appleturl,
                       width, height, 'JME.class', javaparams, jmeoptions, topnode);
    },
    update_inputs : function(name, topnode) {
        var Y = this.Y;
        //U of L Masoum -- get the right name for each  similar applet
        // topnode = "div.que.jme#q" + name.substring(3, name.length);
    ...
    show_error : function (Y, topnode) {
        var errormessage = '<span class ="javascriptwarning">' +
            M.util.get_string('enablejavascript', 'qtype_jme') +
            '</span>';
    ...
    show_java : function (id, appletid, name, java, width, height, appletclass, javavars, jmeoptions, topnode) {
        ...
        if (typeof JSApplet !== 'object' && this.reload_limit) {
            setTimeout(function(){M.qtype_jme.show_java(id, appletid, name,
                            java, width, height, appletclass, javavars, jmeoptions, topnode)}, 100);
            this.reload_limit--;
            return false;
        }
        ...
        if (this.useJSME) {
            ...
            jsmeApplet = new JSApplet.JSME(appletid, width + 'px', height + 'px', {
                // Optional parameters.
        ...
        this.update_inputs(name, topnode);
