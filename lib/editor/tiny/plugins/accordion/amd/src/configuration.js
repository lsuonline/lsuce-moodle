...
import {addToolbarButton, addToolbarSection, addMenubarItem, addContextmenuItem} from 'editor_tiny/utils';
...
export const configure = (instanceConfig, options) => {
    ...
    // Insert menu: accordion insert + remove only (attributes moved to Format).
    const insertMenuItems = `${accordionButton} ${accordionRemoveButton}`;
    ...
    const pluginsConfig = {
        ...
        menu: addMenubarItem(
            addMenubarItem(instanceConfig.menu, 'insert', insertMenuItems),
            'format',
            attributesButtonName
        ),
        contextmenu: addContextmenuItem(instanceConfig.contextmenu, attributesButtonName),
    };

    // Only add toolbar icons if showtoolbaricons is enabled.
    // Attributes button is intentionally excluded from the main toolbar —
    // it is accessible via the right-click context menu and the inline
    // context toolbar that appears when the cursor is inside an accordion.
