...
$sqlcond = 'sql_(as|bitand|bitnot|bitor|bitxor|cast_char2int|ceil|compare_text|concat|concat_join|empty|fullname|';
$sqlcond .= 'ilike|isempty|isnotempty|length|max|null_from_clause|order_by_text|paging_limit|position|substr)';
$helper = array(
...
// Define some known false positives to take them out from errors report.
// (nested array of => file => regular expressions considered false positives).
$fp = array(
                'install.php' => array(
                ...
                'admin/health.php' => array(
                                '\. \$CFG-\>prefix \.'
                // Health script is allowed to use $CFG->prefix when building suggested SQLs to be shown.
                ),
                ...
                'blocks/html/backup/moodle2/restore_html_block_task.class.php' => array(
                // 1-verified expression returning recordset.
                'return.*get_recordset_sql'
                ),
...
$excludes .='MetaIndexes|MetaColumns|MetaColumnNames|MetaPrimaryKeys|)|protected \$[a-zA-Z]*db|Incorrect |';
$excludes .='check find_index_name|not available anymore|output|Replace it with the correct use of|where order of parameters is|';
$excludes .='_moodle_database|invaliddbtype|has been deprecated in Moodle 2\.0\. ';
$excludes .='Will be out in Moodle 2\.1|Potential SQL injection detected|requires at least two parameters|';
$excludes .='hint_database = install_db_val|';
$excludes .='Current database \(|admin_setting_configselect|(if|while|for|return).*\>';
$excludes .='get_recordset(_list|_select|_sql)?.*\>valid\(\)|NEWNAMEGOESHERE.*XMLDB_LINEFEED)/';
// Getting current dir.
...
$dml_megarule         = calculate_megarule($dml, array(
                '[ =@.]'
), array(
                '( )?\('
), 'i');
$helper_megarule      = calculate_megarule($helper, array(
                '[ =@.]'
), array(
                '( )?\('
), 'i');
$ddl_megarule         = calculate_megarule($ddl, array(
                '[ =@.]'
), array(
                '( )?\('
), 'i');
$coreonly_megarule    = calculate_megarule($coreonly, array(
                '[ =@.]'
), array(
                '( )?\('
), 'i');
$enum_megarule        = calculate_megarule($enum);
$internal_megarule    = calculate_megarule($internal, array(
                '[ =@.]'
), array(
                '( )?\('
), 'i');
$unsupported_megarule = calculate_megarule($unsupported, array(
                '[ \>=@,.]'
), array(
                '( )?\('
));
 ...
 * @return mixed The results as array of paths.
 */
function files_to_check($path) {
    ...
    while (false !== ($file = readdir($dir))) {
        ...
        if (is_file($fullpath) &&
                        ...
                        strpos($file, '.xml') === false) { // Exclude some files.
            continue;
        }
