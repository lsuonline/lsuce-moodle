...
class TurningTechDeviceMap extends TurningModel {
    ...
    public static function getalldevices($user, $course = false) {
        ...
        $sqls = "SELECT d.id AS devicemapid, u.id as userid, d.data as deviceid,'1'as typeid,{$course->id} as courseid, '1' as all_courses,  
        u.deleted, 'Response Card' as devicetype
        FROM {user_info_data} d
		INNER JOIN {user} u ON u.id = d.userid
        WHERE u.id = $user->id AND d.fieldid = :fieldid AND u.deleted = 0 ";
        $params['fieldid'] = $fieldid;
        if ($records = $DB->get_records_sql($sqls, $params)) {
